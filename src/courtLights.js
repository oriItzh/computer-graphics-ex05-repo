// Court lighting dimensions and properties
const LIGHT_POLE_HEIGHT = 8; // Height of the lighting poles in meters
const LIGHT_POLE_RADIUS = 0.15; // Radius of the lighting poles in meters
const LIGHT_POLE_DISTANCE = 3; // Distance from court sidelines in meters
const LIGHT_INTENSITY = 0.6; // Base intensity of the spotlights

// Lamp fixture dimensions
const LAMP_WIDTH = 1.5;
const LAMP_HEIGHT = 0.5;
const LAMP_DEPTH = 0.7;
const LIGHT_PANEL_RADIUS = 0.15;
const LIGHT_PANEL_SPACING = 0.4; // Space between light panels in meters

export function createCourtLighting(scene, COURT_LENGTH, COURT_WIDTH) {
  // Materials for different parts of the light fixtures
  ...
  const LIGHT_POLE_SPACING = COURT_LENGTH / 3; // Space poles evenly along court length

  // Create light poles on both sides of the court
...
function createSingleCourtLight(poleX, poleZ, side, poleMaterial, lampMaterial, lightPanelMaterial, COURT_WIDTH) {
  ...
  // Create three light panels on the lamp
  for (let j = -1; j <= 1; j++) {
  ...
  spotlight.angle = Math.PI / 6; // 30 degrees beam angle
  spotlight.penumbra = 0.3; // Soft edges
  spotlight.decay = 0.8; // Light falloff
  spotlight.distance = 25; // Maximum light distance
  spotlight.castShadow = true;
  
  // Configure spotlight target to point at the court
  const targetX = poleX;
